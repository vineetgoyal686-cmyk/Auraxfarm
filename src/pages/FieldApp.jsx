...
export default function FieldApp() {
  ...
  const [farmsView, setFarmsView] = useState('table')
  const [farmsDateRange, setFarmsDateRange] = useState([null, null])
  const [farmsPage, setFarmsPage] = useState(0)
  const [farmsPageSize, setFarmsPageSize] = useState(10)
  const [viewFarmerTab, setViewFarmerTab] = useState('farmer')
  ...
  // One row per farmer who has at least one captured land record, aggregating
  // their land/crop counts and IDs — Land tab shows farmer-wise summaries,
  // not one card per individual land parcel.
  const landRows = useMemo(() => {
    const byFarmer = {}
    for (const farm of farms) {
      if (!byFarmer[farm.farmer_id]) byFarmer[farm.farmer_id] = []
      byFarmer[farm.farmer_id].push(farm)
    }
    return Object.entries(byFarmer).map(([farmerId, farmerFarms]) => {
      ...
      return {
        farmerId,
        farmer,
        totalLand: farmer?.total_farms || farmerFarms.length,
        landRecords: farmerFarms,
        cropCount: farmerCrops.length,
        createdDates: farmerFarms.map((f) => f.created_at).filter(Boolean)
      }
    })
  }, [farms, farmers, crops])

  const [farmsRangeStart, farmsRangeEnd] = farmsDateRange
  const filteredLandRows = landRows.filter((r) => {
    const q = farmsQuery.trim().toLowerCase()
    const matchesQuery =
      !q ||
      r.farmer?.name?.toLowerCase().includes(q) ||
      displayId(r.farmerId).toLowerCase().includes(q) ||
      r.farmer?.village?.toLowerCase().includes(q)
    if (!matchesQuery) return false
    if (!farmsRangeStart || !farmsRangeEnd) return true
    const endOfDay = new Date(farmsRangeEnd.getFullYear(), farmsRangeEnd.getMonth(), farmsRangeEnd.getDate(), 23, 59, 59, 999)
    return r.createdDates.some((d) => {
      const created = new Date(d)
      return created >= farmsRangeStart && created <= endOfDay
    })
  })

  useEffect(() => {
    setFarmsPage(0)
  }, [farmsQuery, farmsDateRange, farmsPageSize])

  const farmsTotalPages = Math.max(1, Math.ceil(filteredLandRows.length / farmsPageSize))
  const farmsCurrentPage = Math.min(farmsPage, farmsTotalPages - 1)
  const pagedLandRows = filteredLandRows.slice(
    farmsCurrentPage * farmsPageSize,
    farmsCurrentPage * farmsPageSize + farmsPageSize
  )
        ...
        <div className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b shrink-0">
          <div className="px-4 lg:px-8 py-3 flex items-center gap-3">
            ...
            {tab === 'farms' && (
              <div className="ml-auto flex items-center gap-2">
                <div className="flex rounded-lg border bg-gray-50 p-1">
                  <button
                    onClick={() => setFarmsView('card')}
                    title="Card view"
                    className={`p-1.5 rounded-md ${farmsView === 'card' ? 'bg-white shadow text-green-700' : 'text-gray-400'}`}
                  >
                    <LayoutGrid className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setFarmsView('table')}
                    title="Table view"
                    className={`p-1.5 rounded-md ${farmsView === 'table' ? 'bg-white shadow text-green-700' : 'text-gray-400'}`}
                  >
                    <List className="w-4 h-4" />
                  </button>
                </div>
          ...
          {tab === 'farms' && (
            <div className="px-4 lg:px-8 pb-3 flex items-center gap-3 flex-wrap">
              ...
              <span className="text-xs font-bold text-gray-500 shrink-0">{filteredLandRows.length} farmers</span>
              <div className="ml-auto">
                <DateRangePicker value={farmsDateRange} onChange={setFarmsDateRange} />
              </div>
            </div>
          )}
        </div>
          ...
          <div className="space-y-4">
            {farmsView === 'card' ? (
              <div className="rounded-lg bg-white border shadow-sm">
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4 rounded-t-lg overflow-hidden">
                  {pagedLandRows.map((r) => (
                    <div key={r.farmerId} className="p-4 rounded-lg bg-white border shadow-sm space-y-3">
                      <div>
                        <div className="font-bold">{r.farmer?.name || 'Unknown'}</div>
                        <div className="text-[11px] text-gray-500 font-mono">{displayId(r.farmerId)}</div>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="p-2 rounded-lg bg-amber-50 border border-amber-100">
                          <div className="text-[9px] font-bold uppercase text-amber-700">Total Land</div>
                          <div className="text-sm font-bold text-amber-800">{r.totalLand}</div>
                        </div>
                        <div className="p-2 rounded-lg bg-green-50 border border-green-100">
                          <div className="text-[9px] font-bold uppercase text-green-700">Land Ids</div>
                          <div className="text-sm font-bold text-green-800">{r.landRecords.length}</div>
                        </div>
                        <div className="p-2 rounded-lg bg-teal-50 border border-teal-100">
                          <div className="text-[9px] font-bold uppercase text-teal-700">Crops</div>
                          <div className="text-sm font-bold text-teal-800">{r.cropCount}</div>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {r.landRecords.map((f) => (
                          <span key={f.id} className="text-[11px] font-mono font-semibold px-2 py-0.5 rounded-md bg-gray-100 text-gray-600">
                            {displayId(f.id)}
                          </span>
                        ))}
                      </div>
                      <div className="flex gap-2 pt-1">
                        <button
                          onClick={() => {
                            setViewFarmerTab('land')
                            setViewFarmer(r.farmer)
                          }}
                          className="flex-1 py-1.5 rounded-full border text-xs font-bold text-gray-600 hover:bg-gray-50 flex items-center justify-center gap-1"
                        >
                          <Eye className="w-3.5 h-3.5" /> View
                        </button>
                        <button
                          onClick={() => setEditFarmer(r.farmer)}
                          className="flex-1 py-1.5 rounded-full border text-xs font-bold text-gray-600 hover:bg-gray-50 flex items-center justify-center gap-1"
                        >
                  ...
                  {filteredLandRows.length === 0 && (
                    <div className="text-sm text-gray-400 col-span-full">No land captured yet.</div>
                  )}
                </div>
                <Pagination
                  page={farmsPage}
                  setPage={setFarmsPage}
                  pageSize={farmsPageSize}
                  onPageSizeChange={setFarmsPageSize}
                  totalItems={filteredLandRows.length}
                />
              </div>
            ) : (
              <div className="rounded-lg bg-white border shadow-sm">
                <div className="overflow-x-auto rounded-t-lg">
                  <table className="w-full min-w-[900px] text-sm border-collapse whitespace-nowrap">
                    <thead className="text-[11px] uppercase text-gray-500 bg-gray-50">
                      <tr className="divide-x divide-gray-300">
                        <th className="text-left px-4 py-3 border-b border-gray-300">Farmer Name</th>
                        <th className="text-left px-4 border-b border-gray-300">Farmer ID</th>
                        <th className="text-left px-4 border-b border-gray-300">Total Land</th>
                        <th className="text-left px-4 border-b border-gray-300">All Land ID</th>
                        <th className="text-left px-4 border-b border-gray-300">Total Crop</th>
                        <th className="text-right px-4 border-b border-gray-300">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {pagedLandRows.map((r) => (
                        <tr key={r.farmerId} className="divide-x divide-gray-200 hover:bg-gray-50/60">
                          <td className="py-3 px-4 font-bold border-b border-gray-200">{r.farmer?.name || 'Unknown'}</td>
                          <td className="px-4 py-3 font-mono text-xs text-gray-600 border-b border-gray-200">{displayId(r.farmerId)}</td>
                          <td className="px-4 py-3 font-mono text-gray-600 border-b border-gray-200">{r.totalLand}</td>
                          <td className="px-4 py-3 border-b border-gray-200 whitespace-normal min-w-[220px]">
                            <div className="flex flex-wrap gap-1">
                              {r.landRecords.map((f) => (
                                <span key={f.id} className="text-[11px] font-mono font-semibold px-2 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-amber-700">
                                  {displayId(f.id)}
                                </span>
                              ))}
                            </div>
                          </td>
                          <td className="px-4 py-3 font-mono text-gray-600 border-b border-gray-200">{r.cropCount}</td>
                  ...
                  {filteredLandRows.length === 0 && (
                    <p className="text-xs text-gray-400 py-4 px-4">No land captured yet.</p>
                  )}
                </div>
                <Pagination
                  page={farmsPage}
